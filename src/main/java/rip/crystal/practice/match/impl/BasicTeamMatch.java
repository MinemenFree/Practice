					...
					if (kit.getGameRules().isBoxing()) {
						...
						config.getStringList("FIGHTS.1V1.BOXING-MODE").forEach(line -> {
							lines.add(line.replace("<bars>", bars)
									...
									.replace("<hits-difference>", (yours.getLeader().getHits() >= opponent.getLeader().getHits() ? String.valueOf(yours.getLeader().getHits() - opponent.getLeader().getHits()) : String.valueOf(opponent.getLeader().getHits() - yours.getLeader().getHits())))
								  	.replace("<hits-difference-color>", (yours.getLeader().getHits() >= opponent.getLeader().getHits() ? CC.GREEN : CC.RED))
								  	.replace("<hits-difference-reverse-color>", (yours.getLeader().getHits() >= opponent.getLeader().getHits() ? CC.RED : CC.GREEN))
								  	.replace("<hits-difference-plus-symbol>", (yours.getLeader().getHits() >= opponent.getLeader().getHits() ? "+" : ""))
								  	.replace("<hits-difference-reverse-plus-symbol>", (yours.getLeader().getHits() >= opponent.getLeader().getHits() ? "" : "+"))
								  	.replace("<hits-difference-minus-symbol>", (yours.getLeader().getHits() >= opponent.getLeader().getHits() ? "" : "-"))
								  	.replace("<hits-difference-reverse-minus-symbol>", (yours.getLeader().getHits() >= opponent.getLeader().getHits() ? "-" : ""))
			...
			cPractice.get().getScoreboardConfig().getStringList("FIGHTS.SPECTATING").forEach(s ->
					lines.add(s
							...
							.replace("<playerB>", String.valueOf(participantB.getLeader().getUsername()))
						  	.replace("<playerA-ping>", String.valueOf(PlayerUtil.getPing(participantA.getLeader().getPlayer())))
						  	.replace("<playerB-ping>", String.valueOf(PlayerUtil.getPing(participantB.getLeader().getPlayer())))
			...
			if (line.equalsIgnoreCase("<match-inventories>")) {
				...
				BaseComponent[] winners = generateInventoriesComponents(
					new MessageFormat(Locale.MATCH_END_WINNER_INVENTORY.format(profile.getLocale()))
						.add("<winner>", String.valueOf(winningParticipant))
						.add("<winner_context>", participantA.getPlayers().size() == 1 ? "" : "s"));
				...
				BaseComponent[] losers = generateInventoriesComponents(
					new MessageFormat(Locale.MATCH_END_LOSER_INVENTORY.format(profile.getLocale()))
						.add("<loser>", String.valueOf(losingParticipant))
						.add("<loser_context>", participantB.getPlayers().size() > 1 ? "s" : ""));
