				...
				if (participantA.getPlayers().size() == 1 && participantB.getPlayers().size() == 1) {
					...
					if ((yours.getLeader().getHits() - opponent.getLeader().getHits()) > 0) {
						...
						actualHits = String.valueOf(yours.getLeader().getHits() - opponent.getLeader().getHits());
				            ...
				            comboCounter = config.getStringList("FIGHTS.1V1.BOXING-MODE.YOUR_COMBO").forEach(line -> {
					    ...
					    comboCounter = config.getStringList("FIGHTS.1V1.BOXING-MODE.OPPONENT_COMBO").forEach(line -> {
							lines.add(line.replace("<your-combo>", String.valueOf(yours.getLeader().getCombo()));
								        .replace("<opponent-combo>", String.valueOf(opponent.getLeader().getCombo())));
						});
						return lines;
                                        } else if ((opponent.getLeader().getCombo() <= 0 && yours.getLeader().getCombo() <= 0)) {
				            comboCounter = config.getStringList("FIGHTS.1V1.BOXING-MODE.NO_COMBO").forEach(line -> {
							lines.add(line.replace("<your-combo>", String.valueOf(yours.getLeader().getCombo()));
								        .replace("<opponent-combo>", String.valueOf(opponent.getLeader().getCombo())));
						});
						return lines;
